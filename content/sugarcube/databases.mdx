...
To store any data in MongoDB we can use the [`mongodb_store`](/sugarcube/plugins/mongodb_store) plugin. You only have to point the plugin to the right server and database using the `--mongodb.uri` option. A MongoDB URI starts with `mongodb://` to define that it is targeting Mongodb, followed by the host name of the server (if you run MongoDB on your local computer it is `localhost`) and the name of the database (in this case `sugarcube-project`).
...
- [`mongodb_complement`](/sugarcube/plugins/mongodb_complement)
- [`mongodb_supplement`](/sugarcube/plugins/mongodb_supplement)
...
- [`mongodb_fetch_units`](/sugarcube/plugins/mongodb_fetch_units)
- [`mongodb_query_units`](/sugarcube/plugins/mongodb_query_units)
