...
When you run a Sugarcube pipeline data gets generated and transformed while the pipeline is running but is lost as soon as the pipeline finishes. In order to store data permanetly a database is required. Sugarcube currently supports two databases, [MongoDB](https://www.mongodb.com/) and [Elasticsearch](https://www.elastic.co/products/elasticsearch).
